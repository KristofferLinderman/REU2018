...
if (os.path.isfile('data/' + subject + '/' + video +'/diagnostics.txt')) == False:
    print("Diagnostics file not found. Run diagnostics first!")
    sys.exit()
else:
    diag = open('data/' + subject + '/' + video +'/diagnostics.txt', 'r')
    z = 0
    for line in diag:
        if z == 0:
            z += 1
...
f = open('data/' + subject + '/' + video +'/timedata.txt', 'w')
e = open('data/' + subject + '/' + video +'/eyedata.txt', 'w')
hr = open('data/' + subject + '/' + video +'/heartdata.txt', 'w')
sm = open('data/' + subject + '/' + video +'/smiledata.txt', 'w')
c = open('data/' + subject + '/' + video +'/clicker.txt', 'w')
all = open('data/' + subject + '/' + video +'/alldata.txt', 'w')
raw = open('data/' + subject + '/' + video +'/raw.txt', 'w')
        ...
        for ((x, y, w, h), n) in faces:
            ...
            if smiles:
                ...
                for smile in smiles:
                    ...
                    cv.PutText(image, str(math.trunc(sizer ** 2)), (smile[0][0], smile[0][1] + smile[0][3] + 10), font,
                               cv.RGB(0, 0, 255))
            ...
            if eyes:
                ...
                for eye in eyes:
                    ...
                    cv.Rectangle(image, (eye[0][0], eye[0][1]), (eye[0][0] + eye[0][2], eye[0][1] + eye[0][3]),
                                 cv.RGB(0, 0, 255), 1, 8, 0)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ret == True:
            x = str(datetime.datetime.now())
            cv2.putText(frame, str(md.datestr2num(x)), (10, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            cv2.putText(frame, x, (10, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            App.out.write(frame)
            #######################################
